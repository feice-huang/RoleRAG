...
PROCESS_DIR = "/data/hfc/RoleRAG/data0506/process"
OUTPUT_DIR = "/data/hfc/RoleRAG/data0506/all"



def cot2all(world, role):
    """
    ...
    # 定义需要遍历的子文件夹
    subfolders = ["cot", "anti_cot"]
    merged_data = []

    # 遍历子文件夹
    for subfolder in subfolders:
        folder_path = os.path.join(PROCESS_DIR, subfolder)
        if not os.path.exists(folder_path):
            continue

        # 查找以 {world}_{role} 开头的 JSON 文件
        pattern = os.path.join(folder_path, f"{role}_cot*.json")
    ...
    output_file = os.path.join(OUTPUT_DIR, f"{world}_{role}_cot_{count}.json")
