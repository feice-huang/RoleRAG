...
def generate_qa_for_scene(scene_data, character, model_name):
    """
    为单个场景生成问答对
...
def generate_qa_for_character(scenes_file, character, model_name, output_file):
    """
    为特定角色的所有场景生成问答对
    ...
    for scene in tqdm(scenes, desc=f"为'{character}'生成问答对"):
        qa_pair = generate_qa_for_scene(scene, character, model_name)
